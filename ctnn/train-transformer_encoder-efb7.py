...
#     def call(self, q, k, v, mask):
    def call(self, q, k, v):
        ...
        scaled_attention_outputs, attention_weights =         scaled_dot_product_attention(q, k, v)
...
y = keras.layers.MaxPool2D(pool_size=(4,1))(b7.get_layer(name='block4a_expand_bn').output)
...
checkpoint = callbacks.ModelCheckpoint("./models_transform/epoch.{epoch:03d}-loss.{loss:.2f}-val_loss.{val_loss:.2f}-b7.h5", monitor='val_loss', verbose=0, save_best_only=True, save_weights_only=True, mode='auto', period=1)
...
model.save("./models_transform/saved_model-b7.h5")
model.save_weights("./models_transform/saved_model_weights-b7.h5")
